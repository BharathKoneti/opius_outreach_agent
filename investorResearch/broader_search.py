...
def search_enterprise_ai_investors(investors):
    """
    Find investors focused on enterprise AI applications including agents, assistants, automation, and more
    """
    # Define enterprise AI related categories - broader definition
    enterprise_ai_categories = [
        "ai agent", "business automation", "enterprise search", 
        "conversation ai", "assistant", "service intelligence",
        "ai security", "mlops", "legal ai", "research assistant"
    ]
    
    ai_investors = []
    
    for investor in investors:
        # Check if Enterprise is in focus areas or if they have enterprise-focused investments
        is_enterprise_focused = any("enterprise" in area.lower() for area in investor["focus_areas"])
        
        # Filter investments based on enterprise AI categories
        ai_investments = []
        
        for investment in investor["investments"]:
            is_match = False
            
            # Check if the AI category matches any of our enterprise AI categories
            for category in enterprise_ai_categories:
                if category.lower() in investment["ai_category"].lower():
                    is_match = True
                    break
                    
            # Only consider investments from the last 3 years (more inclusive)
        ...
        if ai_investments:
            ...
            investor_copy["is_enterprise_focused"] = is_enterprise_focused
            ai_investors.append(investor_copy)
    
    # Sort by enterprise focus (primary) and number of matching AI investments (secondary)
    return sorted(ai_investors, key=lambda x: (x["is_enterprise_focused"], x["ai_investment_count"]), reverse=True)
...
def display_investors(investors):
    ...
    print(f"\n{'=' * 80}")
    print(f"{'INVESTORS FUNDING ENTERPRISE AI APPLICATIONS':^80}")
    print(f"{'=' * 80}")
    
    for idx, investor in enumerate(investors, 1):
        enterprise_focus = "✓" if investor["is_enterprise_focused"] else "✗"
        
        print(f"\n{idx}. {investor['name']} (Enterprise Focus: {enterprise_focus})")
        print(f"   Focus Areas: {', '.join(investor['focus_areas'])}")
        print(f"   Enterprise AI Investments: {investor['ai_investment_count']}")
        print(f"   Recent Investments:")
...
def export_to_csv(investors, filename="enterprise_ai_investors.csv"):
    """
    ...
    with open(filename, "w", newline="") as f:
        ...
        writer.writerow(["Investor Name", "Focus Areas", "Enterprise Focus", "AI Investment Count"])
        ...
        for investor in investors:
            writer.writerow([
                ...
                "Yes" if investor["is_enterprise_focused"] else "No",
                investor["ai_investment_count"]
            ])
    
    # Create a separate CSV for detailed investment data
    with open("enterprise_ai_investments.csv", "w", newline="") as f:
        writer = csv.writer(f)
        
        # Write header for investments
        writer.writerow(["Investor Name", "Enterprise Focus", "Company", "Round", "Amount", "Date", "AI Category"])
    ...
    print(f"Detailed investment data exported to enterprise_ai_investments.csv")

def main():
    print("Searching for investors funding enterprise AI applications...")
    investors = fetch_expanded_investor_data()
    
    # Find investors focused on enterprise AI
    enterprise_ai_investors = search_enterprise_ai_investors(investors)
    
    print(f"\nFound {len(enterprise_ai_investors)} investors funding enterprise AI applications")
